 ...
 * Date        :
 * Description : collect the sum of an array using threads to get the partial sum of even and odd indexes
...
void *sum_array(void *ptr) {
	thread_args* m = (thread_args*) ptr;
	int partial_sum = 0;
	int arr_count = m->start_index;
	for(int i = 0; i < m->length; i++){
		partial_sum += array[arr_count];
		//printf("%d\n",partial_sum)
		arr_count +=2;
	}
    int retval;
...
int main(int argc, char *argv[]) {
    ...
    for(int i = 0; i<2; i++){
    	...
    	//int x = num_values%2;
    	//array can be an odd or even length
    	if((num_values%2) == 1){
    		if(i == 0){
