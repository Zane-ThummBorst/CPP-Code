 ...
 * Date        :
 * Description : sleeps for specified time, or exits early with SIGINT and displays time it was able to sleep.
...
int main(int argc, char *argv[]) {
    ...
    int s = 0;
    while(true){
    	if (signal_val == SIGINT || s == sleep_secs) {
    		break;
    	}
    	// Sleep use to be at the top of the loop and it would return zero if you input ^C quick enough.
    	// It seemed to work all the same where it properly exited on ^C or it reached max seconds.
    	// However Professor Borowski said his code did not output zero so I moved sleep down here.
    	sleep(1);
    	s++;
    }
