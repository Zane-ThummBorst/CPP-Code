 ...
 * Date        :
 * Description : while running a server binary, finds that server from a range of servers to receive a message.
...
int main() {
    ...
    for(int i = 1204; i <= 65535; i++){
    	...
    	//had it before so that it was < 0, which is incorrect because we are trying to connect successfully in this case.
    	// so >= is used instead.
