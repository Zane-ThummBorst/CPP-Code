 ...
 * Works on strings.
 */
linked_list *find_less_str(linked_list *list, char *key, int (*cmp)(const void *, const void *)) {
...
linked_list *find_greater_str(linked_list *list, char *key, int (*cmp)(const void *, const void *)) {
    linked_list *results = NULL;
    for (node *cur = list->head; cur != NULL; cur = cur->next) {
        if (cmp(&cur->data, &key) > 0) {
            if (!results) {
...
int main() {
    ...
    linked_list *less = find_greater_str(list, "horses", str_cmp);
    puts("Search results:");
    if (less) {
